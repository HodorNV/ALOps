```yaml
    - task: ALOpsAICli@3
      ...
      inputs:
        ...
        foundry_resource:                     # Azure AI Foundry resource name (plain name only, e.g. 'iFactoAI' — no URL, no slashes). When non-empty, switches the step into 'foundry' auth mode. Not supported for Copilot in MVP. $(foundry_resource) $(foundry_resource) $(foundry_resource)
