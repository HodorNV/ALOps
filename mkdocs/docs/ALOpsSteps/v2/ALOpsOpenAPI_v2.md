```yaml
    - task: ALOpsOpenAPI@2
      ...
      inputs:
        ...
        publish_artifacts: True               # Publish results as Artifacts. $(publish_artifacts) $(publish_artifacts)
        discover: True                        # Discover available API routes. $(discover) $(discover)
        api_routes:                           # Specify API Routes to convert. Multiline or '|' seperated. $(api_routes) $(api_routes)
        api_exclude_routes:                   # Specify API Routes to exclude from discovery. Multiline or '|' seperated. $(api_exclude_routes) $(api_exclude_routes)
        inspect_paths: False                  # Print all available Paths per Route. $(inspect_paths) $(inspect_paths)
        api_servers:                          # Specify available servers for OpenAPI. Multiline or '|' seperated. $(api_servers) $(api_servers)
        export_edmx: False                    # Export original API EDMX. $(export_edmx) $(export_edmx)
        export_yaml: True                     # Export API in YAML format. $(export_yaml) $(export_yaml)
        export_json: False                    # Export API in JSON format. $(export_json) $(export_json)
        exclude_company_paths: False          # Exclude Company Paths. $(exclude_company_paths) $(exclude_company_paths)
```
