```yaml
    - task: ALOpsExtensionAPI@2
      ...
      inputs:
        ...
        interaction: get                      # Set Interaction Method to use. (Get/Publish/Batch Publish). $(interaction) $(interaction)
        ...
        dev_endpoint:                         # Set DEV Endpoint. (protocol://host:port/serverinstance/dev) $(dev_endpoint) $(dev_endpoint)
        dev_schemeupdatemode: synchronize     # Set Schema Update Mode to use with DEVPort deploy. (synchronize/recreate/forcesync) $(dev_schemeupdatemode) $(dev_schemeupdatemode)
        dev_dependencypublishingoption: none  # Set Dependency Publishing Option for DEV Endpoint. $(dev_dependencypublishingoption) $(dev_dependencypublishingoption)
        ...
        artifact_path: $(System.ArtifactsDirectory)# Path for App Artifact. If using 'Multistate Pipeline', use '$(Pipeline.Workspace)' instead. $(artifact_path) $(artifact_path)
        artifact_filter: *.app                # Filter used for locating App file relative to $(path_to_publish). $(artifact_filter) $(artifact_filter)
        showdeploymentstatus: True            # Show Extension Deployment Status. $(showdeploymentstatus) $(showdeploymentstatus)
        vsix_only: False                      # Only use VSIX, do not use BC-Artifacts. $(vsix_only) $(vsix_only)
        checksecondsdelay: 30                 # Seconds of delay betweel deployment status checks. $(checksecondsdelay) $(checksecondsdelay)
        maxtries: 20                          # Max tries for status check. $(maxtries) $(maxtries)
        replacepackageid: False               # Force a new PackageID for each deployment. $(replacepackageid) $(replacepackageid)
        blocksymbolsonly: False               # Check App, block if SymbolsOnly App. $(blocksymbolsonly) $(blocksymbolsonly)
        schedule: current                     # Set the Schedule. Requires API Version 2.0 $(schedule) $(schedule)
        schema_sync_mode: add                 # Set the Schema Update Mode. Requires API Version 2.0 $(schema_sync_mode) $(schema_sync_mode)
