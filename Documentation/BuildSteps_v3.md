  ...
  * Interact with the Business Central SaaS Admin Center API. Supports environment management (list, get, copy, delete, rename), app lifecycle (install, uninstall, update), and configuration (Application Insights key, security group, update window, scheduled environment updates). Authenticates via OAuth2 client credentials, client certificate, or user impersonation.
  * YAML Template: 
    ```yaml
        - task: ALOpsAdminCenterAPI@3
          ...
          inputs:
            ...
            interaction: environment_list         # Admin Center API operation to execute: environment_list or environment_get reads environment info; environment_copy or environment_rename manages environments; environment_update_schedule selects the target version and date for the next environment update; app_install, app_uninstall, or app_update manages extensions; set_application_insightskey, set_security_group or set_update_window applies configuration. $(interaction) $(interaction) $(interaction)
            api_version: v2.28                    # Version of the Business Central Admin Center API used for all interactions, for example 'v2.28'. Leave empty to use the built-in default. The set_update_window interaction requires v2.13 or later. See https://learn.microsoft.com/en-us/dynamics365/business-central/dev-itpro/administration/administration-center-api for the available versions. $(api_version) $(api_version) $(api_version)
            ...
            update_window_start:                  # Start of the environment update window in 24-hour 'HH:mm' notation, for example '22:00'. Used with the set_update_window interaction. $(update_window_start) $(update_window_start) $(update_window_start)
            update_window_end:                    # End of the environment update window in 24-hour 'HH:mm' notation, for example '06:00'. The window may span midnight and must be at least 6 hours long. Used with the set_update_window interaction. $(update_window_end) $(update_window_end) $(update_window_end)
            update_window_timezone:               # Windows time zone id in which the update window times are expressed, for example 'W. Europe Standard Time', 'Romance Standard Time' or 'UTC'. Required for the set_update_window interaction. Valid values are returned by the Admin Center API route /applications/settings/timezones. $(update_window_timezone) $(update_window_timezone) $(update_window_timezone)
            update_target_version:                # Business Central version to select for the next environment update, for example '27.1', as returned by the Admin Center API updates list. Leave empty to select the latest released (GA) version. Used with the environment_update_schedule interaction. $(update_target_version) $(update_target_version) $(update_target_version)
            update_datetime:                      # Date or date-time (ISO 8601, UTC assumed when no offset is given) on or after which the environment update runs, for example '2026-09-12' or '2026-09-12T22:00:00Z'. A date without time is scheduled at 12:00 UTC. Leave empty to schedule the update as soon as the update window allows. Used with the environment_update_schedule interaction. $(update_datetime) $(update_datetime) $(update_datetime)
            ignore_update_window: false           # When enabled, the scheduled environment update starts at the selected date-time even when that is outside the update window. Used with the environment_update_schedule interaction. $(ignore_update_window) $(ignore_update_window) $(ignore_update_window)
    ...
    ```yaml
        - task: ALOpsAppSource@3
          ...
          inputs:
            ...
            seller_id:                            # Optional numeric Partner Center Seller ID (account ID). When provided, it is sent as the 'x-ms-publisherId' header on every Partner Center (api.partner.microsoft.com) API call. Leave empty to omit the header. $(seller_id) $(seller_id) $(seller_id)
    ...
    ```yaml
        - task: ALOpsAppCompiler@3
          ...
          inputs:
            ...
            alcodeanalyzer:                       # Comma-separated list of code analyzers. Built-in: `CodeCop`, `UICop`, `AppSourceCop`, `PTECop` (alias `PerTenantExtensionCop`). Use `NONE` to disable. A path to a custom analyzer DLL is also accepted, but only in `Serial` compilation mode. $(alcodeanalyzer) $(alcodeanalyzer) $(alcodeanalyzer)
            ...
            fail_on_any: false                    # Fail the task on any compiler diagnostic (error, warning, or info-level message). Stricter than failonwarnings. $(fail_on_any) $(fail_on_any) $(fail_on_any)
            ...
            bc_localization: W1                   # Business Central country code for MS symbol resolution (e.g. `W1`, `BE`, `NL`). When the MSSymbols feed has no localized package for the country (for example `IE` has no `Microsoft.Application.ie.symbols`), the W1 package `Microsoft.Application.symbols` is used automatically. $(bc_localization) $(bc_localization) $(bc_localization)
            ...
            outputalclogs: false                  # Echo the raw compiler output and non-diagnostic status lines, including pragma-suppressed and ignore-listed diagnostics. Error, warning and info diagnostics are always shown regardless of this setting. $(outputalclogs) $(outputalclogs) $(outputalclogs)
            alc_error_log: false                  # Write per-app `<AppName>_ALCErrorLog.txt` (alc /errorlog JSON with error, warning and info issues) next to the compiled .app and publish it as artifact 'ALCErrorLog'. In Parallel mode altool has no errorlog option, so ALOps generates the file from the parsed compiler diagnostics. $(alc_error_log) $(alc_error_log) $(alc_error_log)
            publishartifact: true                 # Publish the compiled .app as a build artifact. $(publishartifact) $(publishartifact) $(publishartifact)
            publishxlif: false                    # Publish generated XLIF translation files (*.g.xlf) as a build artifact named 'ALOpsXLIF'. Enables the 'TranslationFile' compiler feature. $(publishxlif) $(publishxlif) $(publishxlif)
            ...
            updateplatform:                       # Override the 'platform' version in app.json before compilation, e.g. "26.0.0.0". Also updates a governing directory.app.props.json when it defines 'platform'. Empty = keep. $(updateplatform) $(updateplatform) $(updateplatform)
            updateapplication:                    # Override the 'application' version in app.json before compilation, e.g. "26.6.0.0". Also updates a governing directory.app.props.json when it defines 'application'. Empty = keep. $(updateapplication) $(updateapplication) $(updateapplication)
