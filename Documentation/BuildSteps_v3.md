    ...
    ```yaml
        - task: ALOpsAppCompiler@3
          ...
          inputs:
            ...
            fail_on_any: false                    # Fail the task on any compiler diagnostic (error, warning, or info-level message). Stricter than failonwarnings. In Parallel mode this also raises the compile log level so info diagnostics are captured. $(fail_on_any) $(fail_on_any) $(fail_on_any)
