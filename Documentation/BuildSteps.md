    ...
    ```yaml
        - task: ALOpsAppCompiler@1
          ...
          inputs:
            ...
            al_analyzer:                          # Comma-separated list of AL analyzers to run during compilation, for example CodeCop,UICop,AppSourceCop,PerTenantExtensionCop. Custom analyzers are given by DLL file name, for example BusinessCentral.LinterCop.dll or ALCops.LinterCop.dll; the DLL must be located below the working folder (System.DefaultWorkingDirectory). With usedocker enabled the DLL is copied into the container automatically. Leave empty to use only the default set of analyzers. $(al_analyzer)
            ...
            setup_working_folder: False           # Copy the local working folder into the Docker container before compilation starts. Required when the container does not already contain the source files. When disabled, the compiler uses the copy made by ALOps Docker Wait; files added to the working folder after that step are not visible inside the container. $(setup_working_folder)
            updateplatform:                       # Updates the 'platform' version in app.json before compilation, e.g. '26.0.0.0'. Empty = keep app.json value. $(updateplatform)
            updateapplication:                    # Updates the 'application' version in app.json before compilation, e.g. '26.6.0.0'. Empty = keep app.json value. $(updateapplication)
            ...
            additionalprobingpaths:               # One additional .NET assembly probing path per line. The AL compiler uses these paths to locate external .NET assemblies referenced by the app. Relative paths are resolved against the working folder (inside the container when usedocker is enabled). Paths that do not exist are skipped with a warning. Not needed for analyzer DLLs: every folder below the working folder that contains a DLL is added automatically. $(additionalprobingpaths)
